...
​	配置之后，需要 `npm install`，直接 `npm link` 也能更新依赖。但发布时需要具备像`lerna publish` 将本地连接解析成线上连接的功能对依赖进行处理。
...
- `npm` `file:` 接相对路径的处理
  ...
  ![](./images/resolveLocalDependencyLinks.jpg)
